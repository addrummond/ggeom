...
func SegmentLoopIntersections(points []Vec2) []Intersection {
	...
	for e, notEmpty := events.Pop(); notEmpty; e, notEmpty = events.Pop() {
		...
		if event.kind == start {
			...
			fmt.Printf("Appending the intersection of %v and %v to result\n", event.i, event.i2)
