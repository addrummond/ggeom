...
func TestConvolve(t *testing.T) {
	...
	for i := 0; i < len(tests); i += 3 {
		...
		for _, pt := range cs {
			fmt.Printf("{{{\n")
			fmt.Printf("    %+v\n", pt.y)
			fmt.Printf("    %v/%v, %v/%v\n", pt.x.Num(), pt.x.Denom(), pt.y.Num(), pt.y.Denom())
			fmt.Printf("        %v, %v\n", pt.ApproxX(), pt.ApproxY())
			fmt.Printf("}}}\n")
		}
		...
		for _, itn := range itns {
			...
			//fmt.Printf("    (%v,%v) -> (%v,%v)  with  (%v,%v) -> (%v,%v)  at (%v,%v)\n", cs[itn.seg1].x.String(), cs[itn.seg1].y.String(), cs[(itn.seg1+1)%len(cs)].x.String(), cs[(itn.seg1+1)%len(cs)].y.String(), cs[itn.seg2].x.String(), cs[itn.seg2].y.String(), cs[(itn.seg2+1)%len(cs)].x.String(), cs[(itn.seg2+1)%len(cs)].y.String(), itn.p.x.String(), itn.p.y.String())
		}
		fmt.Printf("DONE PRINTING RAW\n")*/
